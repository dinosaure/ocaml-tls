...
let cipher_to_string cipher = List.find (fun (_, x) -> x = cipher) ciphers |> fst
let cipher_of_string s = try List.assoc s ciphers with _ -> invalid_arg "Invalid cipher: %s" s
...
let algorithm_to_string algorithm = List.find (fun (_, x) -> x = algorithm) algorithms |> fst
let algorithm_of_string s = try List.assoc s algorithms with _ -> invalid_arg "Invalid algorithm: %s" s
let algorithm_pp ppf = Fmt.string ppf <.> algorithm_to_string

let ciphersuite algorithm cipher : Tls.Ciphersuite.ciphersuite = match cipher, algorithm with
  ...
  | `RC4_128_MD5,        `DHE_RSA
  | `RC4_128_SHA,        `DHE_RSA -> invalid_arg "Ciphersuite unavailable"
...
let hash_of_string s = try List.assoc s hashes with _ -> invalid_arg "Invalid hash: %s" s
...
let to_own_cert own_cert : (Tls.Config.own_cert, [ `Msg of string ]) result Async.Deferred.t =
  let list_of_result_to_result_of_list lst =
    try Rresult.R.ok (List.map (function Error err -> raise (Jump err) | Ok v -> v) lst)
...
let check main host port reneg certificates ca_file ca_path ciphers hashes =
  let authenticator = match ca_file, ca_path with
    ...
    | Some _, Some _ -> `Error (true, "Impossible to load both CA file and CA directory.") in

  match authenticator with
  | `Error _ as err -> err
  | #authenticator as authenticator ->
    match Async.Thread_safe.block_on_async (fun () -> main host port reneg certificates authenticator ciphers hashes) with
    | Ok v -> v
    | Error exn -> `Error (false, Fmt.strf "Got an exception during executation: %s" (Core.Exn.to_string exn))
...
let ffile =
  let parse path =
    ...
    then Fpath.of_string path
    else Rresult.R.error_msgf "File %s does not exist" path in
  let pp = Fpath.pp in
...
let path =
  let parse path =
    ...
    then Fpath.of_string path
    else Rresult.R.error_msgf "Path %s is not a directory" path in
  let pp = Fpath.pp in
...
let cipher =
  let parse s =
    try Rresult.R.ok (cipher_of_string s)
    with Invalid_argument err -> Rresult.R.error_msg err in
  let pp = cipher_pp in
  Arg.conv (parse, pp) ~docv:"<cipher>"

let algorithm =
  let parse s =
    try Rresult.R.ok (algorithm_of_string s)
    with Invalid_argument err -> Rresult.R.error_msg err in
...
let ciphersuite =
  let parse s = match Astring.String.cut ~sep:":" s with
    ...
    | Some (a, c) -> match Arg.conv_parser algorithm a, Arg.conv_parser cipher c with
      | (Error _ as err), _ | _, (Error _ as err) -> err
      | Ok a, Ok c ->
        try Rresult.R.ok (ciphersuite a c)
        with Invalid_argument err -> Rresult.R.error_msg err in
  let pp = Fmt.using split_ciphersuite
      (Fmt.pair ~sep:(Fmt.const Fmt.string ":") algorithm_pp cipher_pp) in
  Arg.conv ~docv:"<algorithm>:<cipher>" (parse, pp)

let hash : Nocrypto.Hash.hash Arg.conv =
  let parse s =
    try Rresult.R.ok (hash_of_string s)
    with Invalid_argument err -> Rresult.R.error_msg err in
  let pp = hash_pp in
...
let certchain =
  let parse s = match Astring.String.cut ~sep:":" s with
    | Some (cert, priv_key) ->
      if Sys.file_exists cert && Sys.file_exists priv_key
      then Rresult.R.(Fpath.of_string cert
                      >>= fun cert -> Fpath.of_string priv_key
                      >>= fun priv_key -> return (cert, priv_key))
      else Rresult.R.error_msgf "Certificate file or private key file don't exist: %s or %s"
          cert priv_key
    | None -> Rresult.R.error_msgf "Invalid format of certchain: %s" s in
    ...
    let rest_parse = function
      ...
      | own_cert ->
        let own_cert =
          ...
          |> List.fold_left (fun acc -> function Ok v -> v :: acc | Error _ -> acc) [] in
        Rresult.R.ok (`Multiple own_cert) in
    match Astring.String.cut ~sep:"!" s with
    | None ->
      (try rest_parse (Astring.String.cuts ~sep:"," s)
       with _ -> Rresult.R.error_msgf "Invalid format of own-cert: %s" s)
    | Some (default, rest) ->
      let open Rresult.R in
      parse_certchain default
      >>= fun default ->
      (try rest_parse (Astring.String.cuts ~sep:"," rest)
       with _ -> Rresult.R.error_msgf "Invalid format of own-cert: %s" rest)
      >>| function
      | `Single rest -> `Multiple_default (default, [ rest ])
      | `Multiple rest -> `Multiple_default (default, rest) in
  let pp ppf = function
    | `Multiple lst -> Fmt.pf ppf "(`Multiple %a)" Fmt.(list ~sep:(const string ",") pp_certchain) lst
    | `Single v -> Fmt.pf ppf "(`Single %a)" pp_certchain v
    | `Multiple_default (v, lst) ->
      Fmt.pf ppf "(`Multiple_default %a)"
        Fmt.(pair ~sep:(const string "!") pp_certchain (list ~sep:(const string ",") pp_certchain))
        (v, lst) in
  Arg.conv (parse, pp)
