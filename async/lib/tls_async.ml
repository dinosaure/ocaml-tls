  ...
  let make_writer fd : [ `Write ] buffer -> [ `Ok of int | `Closed ] Deferred.t =
    ...
    let finish result =
      ...
      | `Error exn ->
        ...
        raise exn in (* XXX(dinosaure): exception leak. *)
    fun ({ Cstruct.buffer = buf; off = pos; len; }) ->
      if Fd.supports_nonblock fd
      then finish (Fd.syscall fd ~nonblocking:true (fun fd -> Bigstring.write_assume_fd_is_nonblocking ~pos ~len fd buf))
      else Fd.syscall_in_thread fd ~name:"writer" (fun fd -> Bigstring.write ~pos ~len fd buf) >>= finish

  let make_reader fd : [ `Read ] buffer -> [ `Ok of int | `Eof ] Deferred.t =
    let cstruct_to_bigstring { Cstruct.buffer = buf ; off = pos; len; } f = f buf ~pos ~len in
    let rec finish fd buf result =
      ...
      | `Error (Unix.Unix_error ((EWOULDBLOCK | EAGAIN), _, _)) ->
        (Fd.ready_to fd `Read >>= function
          | `Bad_fd -> bad_fd ()
          | `Closed -> return `Eof
          | `Ready -> go fd buf) (* XXX(dinosaure): ready to read again. *)
      | `Error (Unix.Unix_error (EBADF, _, _)) ->
        bad_fd ()
      | `Error exn ->
        Deferred.don't_wait_for (Fd.close fd);
        raise exn (* XXX(dinosaure): exception leak. *)
    and go fd buf =
      if Fd.supports_nonblock fd
      then finish fd buf
        @@ Fd.syscall fd ~nonblocking:true
        @@ fun fd -> cstruct_to_bigstring buf
        @@ fun buf ~pos ~len -> Unix.Syscall_result.Int.ok_or_unix_error_exn ~syscall_name:"read"
...
let rd, wr =
  let recording_errors safe_computation t cs =
    safe_computation t.socket cs >>= function
    | Ok res -> return res
    | Error exn ->
      t.state <- `Error exn
    ; raise exn in (* exception leaks *)
...
let rec drain_handshake t =
  let to_linger t mcs =
    ...
    | Some cs, Some linger -> t.linger <- Some (Cstruct.append linger cs) in
  match t.state with
  | `Error exn -> raise exn (* XXX(dinosaure): in any case, we should raise an error. *)
  | `Eof -> return t
  | `Active tls ->
    if not (Tls.Engine.handshake_in_progress tls)
    then return t
    else rd_react t >>= function
